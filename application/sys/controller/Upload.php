 ...
 * 通用上传接口
 * Class Upload
 * @package app\transfer\controller
 */
class Upload extends Controller
{
    ...
    public function upload()
    {
        ...
        $data = $Resource->fileUpload(0,$file);
        if( !empty($data['url']) ){
            $result = [
                'state' => 'SUCCESS',
                'url' => $data['url'],
        ...
        $upload_path = str_replace('\\', '/', PUBLIC_BASE_PATH . 'uploads/image');
        $save_path   = PUBLIC_PATH.'uploads/image/';
