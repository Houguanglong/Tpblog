...
class System extends AdminBase
{
    ...
    public function index($group = 'base')
    {
        ...
        if ($this->request->isPost()) {
            ...
            $listGroup = configGroup();

            if (isset($listGroup[$group])) {
                // 查询该分组下所有的配置项名和类型
                ...
                foreach ($items as $name => $type) {
                    ...
                    $updateData = Db::name('admin_config')->where('name', $name)->find();

                    cache('config:' . $name, $updateData, 'system-config');
                    model('sys/Config')->eventUpdate($updateData);
                    if ($res === false) {
                        break;
                    }
                }
            ...
            $con_list = Db::name('admin_config')->where(['group' => $group, 'status' => 1])->order(['sort' => 'ASC', 'id' => 'ASC'])->select();
    ...
    public function config($group = 'base')
    {
        ...
        */
        ...
        // 表格数据，分页，每页10条
        $configs = Db::name('admin_config')->where(['group' => $group])->order(['sort' => 'ASC', 'id' => 'ASC'])->paginate(tb_config('list_rows', 1, $this->lang_id));  //取出当前语言版本下的所有配置项
...
//             [lang('config_form_status'),'switch','status',$config['status'],lang('config_form_status_place'),lang('config_form_status_tips')],
    ...
    public function conf_update()
    {
        if (request()->isPost()) {
            ...
            if ($res !== false) {
                model('sys/Config')->eventUpdate($data);
                sys_log(lang('comm_update_success'), 1);  //操作结果写入系统日志
                $this->success(lang('comm_update_success'));
            } else {
                sys_log(lang('comm_update_error'), 0);  //操作结果写入系统日志
