...
class Module extends AdminBase
{
    ...
    public function export($name)
    {
        ...
        // 模块导出目录
        $module_dir = ROOT_PATH . 'data/export/module/' . $name;
        // 删除旧的导出数据
        if (is_dir($module_dir)) {
            File::del_dir($module_dir);
        }
        createDir($module_dir);
        // 复制模块目录到导出目录
        ...
        $archive = new PHPZip;
        // 权限修改
        $iterator = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($module_dir),\RecursiveIteratorIterator::SELF_FIRST);
...
INFO;
        // 写入到文件
        $filePath = ROOT_PATH . 'data/export/module/' . $name . '/menus.php';
