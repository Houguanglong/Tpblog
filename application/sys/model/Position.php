...
class Position extends Model
{
    ...
    public function savePosition($data)
    {
        $saveData = [
            ...
            'nav' => empty($data['nav']) ? 0 : $data['nav'],
            'layout' => empty($data['layout']) ? 0 : $data['layout'],
    ...
    {
        try{
            Db::name('sys_position_bind')->where(['type'=>$data[0]['type'],'t_id'=>$data[0]['t_id']])->update(['trash'=>1]);
     ...
     * @return mixed
     */
    public function bindGoodsList($position,$limit = 0)
    {
        $positionDetail = $this->where('status',1)->where('position',$position)->find();
        if( empty($positionDetail) ){
            return [];
        }
        $condition = '`spd`.`position` = "'.$positionDetail['id'].'" and `spd`.`trash` = "0" and `spd`.`status` = "1" and  `spd`.`type` = 1 and `sg`.`trash` = 0 and `sg`.`status` = 1 and `sg`.`is_draft` = "0"';
        if( empty($limit) ){
            $data = Db::name('sys_position_bind')->alias('spd')->join('__SHOP_GOODS__ sg','spd.t_id = sg.id','RIGHT')->where($condition)->field('sg.*')->select();
        }else{
            $data = Db::name('sys_position_bind')->alias('spd')->join('__SHOP_GOODS__ sg','spd.t_id = sg.id','RIGHT')->where($condition)->limit($limit)->field('sg.*')->select();
        }
        return $data;
    }
