...
class Module extends sysModel
{
     ...
     * @return mixed
     */
    public static function module_config($module = '')
    {
        $module = $module ?: request()->module() ?: config('default_module');
        $config = cache('module_config:'.$module);
        if( empty($config) ){
            $config = Db::name('admin_module_config')->where('module',$module)->find();
        }
        return $config;
    }
     ...
     * @throws \Exception
     */
    public function get_module_info($module = '',$value = '')
    {
        if( !empty($value) ){
            return $this->where('name',$module)->value($value);
        }
