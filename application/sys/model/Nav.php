...
class Nav extends SysModel
{
    ...
    public function initialize()
    {
        ...
        $this->name('sys_nav_position')->where('trash',0);
    }
    ...
    public function selectPositions($status = false,$getAll = false)
    {
        ...
        $positions = $this->where($where)->order('sort asc')->order('id desc');
        if( $getAll == true ){
    ...
    public function selectTopPositions($status = false,$getAll = false)
    {
        ...
        if( $getAll == true ){
            $positions = $this->where($where)->order('sort asc')->order('id desc')->select();
        }elseif( $this->needPage == false ){
            $positions = $this->where($where)->order('sort asc')->order('id desc')->paginate($this->limit);
        }else{
            $positions = $this->where($where)->limit($this->limit)->order('sort asc')->order('id desc')->select();
        }
        return $positions;
    }
    ...
    public function getPosition($id = '')
    {
        ...
        $where = ['id'=>$id,'trash'=>0];
        $position = $this->where($where)->find();
        return $position;
    }
    ...
    {
        $where = ['id'=>$id,'trash'=>0];
        $nav =$this->nav_model()->where($where)->find();
        return $nav;
    }
    ...
    public function selectNavs($limit = '')
    {
        ...
        if( empty($limit) ){
            $lists =$this->nav_model()->where($where)->order('sort asc')->order('id desc')->paginate(10);
        }else{
            $lists =$this->nav_model()->where($where)->limit($limit)->order('sort asc')->order('id desc')->select();
        }
        return $lists;
    }
    ...
    public function selectAllNavs($limit = '')
    {
        ...
        if( empty($limit) ){
            ...
            $lists =$this->nav_model()->where($where)->order('sort asc')->order('id desc')->limit($limit)->select();
        }
        return $lists;
    }
    ...
    {
        try{
            $install =$this->nav_model()->insertGetId($data);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
    ...
    {
        try{
            $install =$this->nav_model()->where('id',$id)->update($data);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
    ...
    {
        try{
            $del =$this->nav_model()->where('id','in',$id)->update(['trash'=>1]);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
    ...
    {
        try{
            $del = $this->where('id','in',$id)->update(['trash'=>1]);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
    ...
    {
        try{
            $update =$this->nav_model()->where('id',$id)->update([$field=>$value]);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
    ...
    {
        try{
            $update = $this->where('id',$id)->update([$field=>$value]);
        }catch (\Exception $e){
            return $e->getMessage();
        }
        return true;
     ...
     * @return mixed
     */
    public function getNavList($pid = null,$position = '',$sort = '',$condition = '',$limit = false)
    {
        $where = [];
        $positionWhere = '';
        if( !empty($position) ){
            if( !is_numeric($position) ){
                $positionInfo = $this->where('position',$position)->find();
        ...
        $list =$this->nav_model() ->where(['trash'=>0,'status'=>1])->where($positionWhere)->where($where)->where($condition)->order($sort)->order('sort asc')->limit($limit)->select();
        return $list;
    }
    ...
    public function getNavs($position,$sort = '',$condition = '',$limit = 10)
    {
        ...
        $list =$this->nav_model()->where($where)->where($condition)->order($sort)->order('sort asc')->limit($limit)->select();
        return $list;
    }
    ...
    public function updateLinks()
    {
        ...
        try{
            // 导航
            $NavList = $this->nav_model()->where('trash',0)->select();
            foreach ($NavList as $key=>$position){
                if( $position['link_type'] != 5){
                    ...
                    $this->nav_model()->where('id',$position['id'])->update(['link'=>$url]);
                }
            }
            Db::commit();
        }catch (\Exception $e) {
     ...
     * @return Db
     */
    public function nav_model()
    {
        return Db::name(self::NAV_TABLE)->where('trash',0);
    }
