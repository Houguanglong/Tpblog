...
class Nav
{
     ...
     * @return mixed
     */
    public function getNavList($pid = null,$position = '',$sort = '',$condition = '',$limit = null)
    {
        $where = [];
        $positionWhere = '';
        if( !empty($position) ){
        ...
        $list = Db::name(self::NAV_TABLE) ->where(['trash'=>0,'status'=>1])->where($positionWhere)->where($where)->where($condition)->order($sort)->order('sort asc')->limit($limit)->select();
        return $list;
    }
     ...
     * @return mixed
     */
    public function getNavs($position,$sort = '',$condition = '',$limit = null)
    {
        $where = '`trash`=0 ';
        if( is_numeric($position) ){
